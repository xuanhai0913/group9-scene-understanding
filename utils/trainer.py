...
class Meter:
    ...
    def metric(self, probability, truth):
        ...
        with torch.no_grad():
            ...
            neg_val = neg.mean().item()
            neg = 0.0 if np.isnan(neg_val) else neg_val
            
            dice_pos_val = dice_pos.mean().item()
            dice_pos = 0.0 if np.isnan(dice_pos_val) else dice_pos_val
...
class BCEDiceLoss:
    ...
    def __call__(self, logits, true):
        ...
        if self.bce_weight < 1.:
            ...
            loss -= (1 - self.bce_weight) * torch.log(dice_loss + self.eps)
...
class Trainer(object):
    ...
    def load_model(self, ckpt_name="best_model.pth"):
        ...
        chkpt = torch.load(ckpt_name, map_location=self.device, weights_only=False)
        ...
        try:
            ...
            logging.warning("******** training_meta.pickle.dat not found, using default training parameters ********")
