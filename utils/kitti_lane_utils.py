...
class KittiLaneDataset(CustomDataset):
    ...
    def __init__(self, hard_augs=False, resize=None, orig_size=(375, 1242), **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        if self.phase != "test":
            ...
            if img.shape[:2] != self.orig_size:
                img = cv2.resize(img, (self.orig_w, self.orig_h), interpolation=cv2.INTER_LANCZOS4)
            if mask.shape[:2] != self.orig_size:
                mask = cv2.resize(mask.astype(np.uint8), (self.orig_w, self.orig_h), interpolation=cv2.INTER_NEAREST)
                if len(mask.shape) == 2:
